...
function AssetsPanel({ push }: any) { const [assets, setAssets] = useState<any[]>([]); const [loading, setLoading] = useState(false); const [typeFilter, setTypeFilter] = useState<'all' | 'image' | 'document' | 'other'>('all'); const [keyword, setKeyword] = useState(''); const [view, setView] = useState<'grid' | 'list'>('grid'); const [savingAlt, setSavingAlt] = useState<Record<string, boolean>>({});
  ...
  {view === 'grid' ? <div className="mt-4 grid gap-3 md:grid-cols-3 xl:grid-cols-4">{visibleAssets.map((asset) => <div key={asset.id} className="rounded-xl border border-[var(--border)] p-3">{classify(asset.mime_type || '') === 'image' ? <img src={asset.public_url} alt={asset.original_filename} className="h-28 w-full rounded-lg object-cover" /> : <div className="grid h-28 place-items-center rounded-lg bg-[var(--bg-soft)] text-xs text-[var(--muted)]">{asset.mime_type || 'file'}</div>}<div className="mt-2 truncate text-xs">{asset.original_filename}</div><input placeholder="alt text" defaultValue={asset.alt_text || ''} className="mt-2 w-full rounded border border-[var(--border)] bg-transparent px-2 py-1 text-xs" onBlur={async (e) => { try { setSavingAlt((s) => ({ ...s, [asset.id]: true })); await api(`/api/assets/${asset.id}`, { method: 'PATCH', body: JSON.stringify({ alt_text: e.target.value }) }); push('success', 'alt text 已保存'); } catch (err: any) { push('error', err.message); } finally { setSavingAlt((s) => ({ ...s, [asset.id]: false })); } }} /><div className="mt-2 flex gap-2"><button className="rounded border px-2 py-1 text-xs" disabled={savingAlt[asset.id]} onClick={async () => { await navigator.clipboard.writeText(markdownFor(asset)); push('success', 'Markdown 已复制'); }}><Copy size={12} className="mr-1 inline" />Markdown</button><button className="rounded border border-rose-400 px-2 py-1 text-xs text-rose-600" onClick={async () => { if (!confirm('确认删除该资源？')) return; await api(`/api/assets/${asset.id}`, { method: 'DELETE' }); await load(); push('success', '资源已删除'); }}><Trash2 size={12} className="mr-1 inline" />删除</button></div></div>)}</div> : <div className="mt-4 overflow-hidden rounded-xl border border-[var(--border)]"><table className="w-full text-sm"><thead className="bg-[var(--bg-soft)] text-left text-xs"><tr><th className="px-3 py-2">名称</th><th className="px-3 py-2">类型</th><th className="px-3 py-2">大小</th><th className="px-3 py-2">操作</th></tr></thead><tbody>{visibleAssets.map((asset) => <tr key={asset.id} className="border-t border-[var(--border)]"><td className="px-3 py-2">{asset.original_filename}<input placeholder="alt text" defaultValue={asset.alt_text || ''} className="mt-1 w-full rounded border border-[var(--border)] bg-transparent px-2 py-1 text-xs" onBlur={async (e) => { try { await api(`/api/assets/${asset.id}`, { method: 'PATCH', body: JSON.stringify({ alt_text: e.target.value }) }); push('success', 'alt text 已保存'); } catch (err: any) { push('error', err.message); } }} /></td><td className="px-3 py-2 text-xs">{asset.mime_type}</td><td className="px-3 py-2 text-xs">{Math.round((asset.file_size || 0) / 1024)}KB</td><td className="px-3 py-2"><div className="flex gap-2"><button className="rounded border px-2 py-1 text-xs" onClick={async () => { await navigator.clipboard.writeText(markdownFor(asset)); push('success', 'Markdown 已复制'); }}>复制</button><button className="rounded border border-rose-400 px-2 py-1 text-xs text-rose-600" onClick={async () => { if (!confirm('确认删除该资源？')) return; await api(`/api/assets/${asset.id}`, { method: 'DELETE' }); await load(); push('success', '资源已删除'); }}>删除</button></div></td></tr>)}</tbody></table></div>}</Panel>; }
