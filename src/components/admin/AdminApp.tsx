...
import { api, ToastView, useToasts } from './admin/shared';
...
export default function AdminApp() {
  ...
  useEffect(() => {
    api('/api/settings').catch(() => {
      // ignore warm-up failures; each panel has its own network error state
    });
  }, []);
...
function AssetsPanel({ push }: any) { const [assets, setAssets] = useState<any[]>([]); const [loading, setLoading] = useState(false); const [typeFilter, setTypeFilter] = useState<'all' | 'image' | 'document' | 'other'>('all'); const [keyword, setKeyword] = useState(''); const [view, setView] = useState<'grid' | 'list'>('grid'); const [savingAlt, setSavingAlt] = useState<Record<string, boolean>>({}); const [altDrafts, setAltDrafts] = useState<Record<string, string>>({});
  ...
  return <Panel title="资源管理" desc="支持筛选、搜索、双视图、alt text 和 Markdown 复制。"><label className="flex min-h-36 cursor-pointer items-center justify-center rounded-2xl border border-dashed border-[var(--border)] p-6 text-sm text-[var(--muted)]">{loading ? '上传中...' : '点击选择文件上传（可多选）'}<input multiple type="file" className="hidden" onChange={(e) => upload(e.target.files)} /></label>
